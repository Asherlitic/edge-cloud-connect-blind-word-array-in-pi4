...
def extract_keywords_with_llm(user_text, ocr_text):
    ...
    if ocr_text:
        ...
        prompt = f"""从以下内容中提取5个左右的核心关键词。

标准：
- 只提取核心名词或短短语：药品名（如复方氨酚烷胺、阿莫西林）、疾病名（如感冒、流感）、症状名（如发热、头痛、咳嗽）、或其他类别的核心概念词
- 每个关键词控制在2~6个字
- 按重要性从高到低排列，用中文逗号分隔
- 只输出关键词本身，不要序号、不要多余文字
...
关键词："""
    ...
    result = ask_smart_assistant(
        prompt_text=prompt,
        system_prompt="你只输出用中文逗号分隔的关键词，每个词2~4个字，不输出完整句子。",
        model="ernie-5.0"
    )
